...
bool ComposeAppEngine::isRunning(const App& app) const {
  try {
    ...
    for (std::size_t i = 0; i < services.size(); i++) {
      ...
      std::string hash;
      if (!compose.getServiceHash(services[i], hash)) {
        LOG_WARNING << "App: " << app.name << "service: " << name << " no hash!";
        return false;
      }
      req += " --filter=label=com.docker.compose.service=" + name;
      req += " --filter=label=io.compose-spec.config-hash=" + hash;

      bool ret{false};
      std::string output;
      std::tie(ret, output) = cmd(docker_ + req);
      if (!ret || output == std::string()) {
        LOG_WARNING << "App: " << app.name << " service: " << name << ", container not running";
        return false;
      }
    }
    return true;
